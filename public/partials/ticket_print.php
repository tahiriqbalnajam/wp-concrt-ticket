...
foreach ( $order->get_items() as $item_key => $item ) {
    ...
    if ($product->is_type('ticket')) {
        ...
        $local_promoter = get_post_meta( $variation_id, 'local_promoter', true );
       ...
       $item_total = intval( $item_data['total'] );
        ...
        for( $i=1; $i<=$quantity; $i++){ 
            ...
            if($order_number == $product_id){
                ...
                $order_encrypt_id = $encp_product_id .'-'. $order_exp_id .'-'.  $order_number . $i;; 
            }
            ?>
                    ...
                    <div class="leftpanel3" style="width: 45%; display: table-cell; vertical-align: top;">
                        ...
                        <span style="display: block; font-family: sans-serif; font-size: 16px; color: orangered;">
                            <?php
                                ...
                                echo 
                                // generating
                                QRcode::png(get_site_url().'/ticket/'.$order_id, $dir_path, QR_ECLEVEL_L, 3);
